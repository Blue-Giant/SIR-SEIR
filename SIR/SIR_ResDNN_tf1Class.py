...
class SIR_model(object):
    ...
    def Solve_SIR(self, S_init=None, I_init=None, R_init=None, iter2SIR=None, t_data=None):
        ...
        """
        betas = self.DNN2beta(t_data)      # [N,1]
        ...
        for it in range(iter2SIR):
            ...
            I_pre = I
            R_pre = R
        return betas, gammas, S_out, I_out, R_out
...
def solve_SIR2COVID(R):
    ...
    size2trainSet = R['Size2Train_Set']
    size2testSet = R['Size2Test_Set']
    batch_size2train = R['batch_size2train']
    batch_size2test = R['batch_size2test']
    ...
    ResDNN2SIR = SIR_model(in_dim=input_dim, out_dim=out_dim, model2DNN=R['model2SIR_paras'],
                            actIn_name=actFunc_name, act_name=actFunc_name, actOut_name=actOutFunc_name)
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            T_batch2train = tf.placeholder(tf.float32, name='T_batch2train', shape=[batch_size2train, input_dim])

            S_true2train = tf.placeholder(tf.float32, name='S_true2train', shape=[batch_size2train, out_dim])
            I_true2train = tf.placeholder(tf.float32, name='I_true2train', shape=[batch_size2train, out_dim])
            R_true2train = tf.placeholder(tf.float32, name='R_true2train', shape=[batch_size2train, out_dim])
            N_true2train = tf.placeholder(tf.float32, name='N_true2train', shape=[batch_size2train, out_dim])
            ...
            T_batch2test = tf.placeholder(tf.float32, name='T_batch2test', shape=[batch_size2test, input_dim])

            pt_penalty = tf.placeholder_with_default(input=1e1, shape=[], name='penalty2predict_true')
            in_learning_rate = tf.placeholder_with_default(input=1e-5, shape=[], name='lr')

            Betas2train, Gammas2train, S_NN2train, I_NN2train, R_NN2train = ResDNN2SIR.Solve_SIR(
            ...
            PWB = penalty2wb * ResDNN2SIR.get_sumWB2DNNmodels()
            loss_sum = pt_penalty*(loss2S + loss2R + loss2I + loss2N) + PWB

            my_optimizer = tf.train.AdamOptimizer(in_learning_rate)
            train_loss = my_optimizer.minimize(loss_sum, global_step=global_steps)

            Betas2test, Gammas2test, S_NN2test, I_NN2test, R_NN2test = ResDNN2SIR.Evaluate_SIR(
    ...
    # 将数据集分为训练集和测试集
    assert(size2trainSet + size2testSet <= len(data))
    trainSet_date, testSet_date = DNN_data.split_1Data2train_test(date, size2train=size2trainSet, normalFactor=1.0)
    trainSet_s, testSet_s = DNN_data.split_1Data2train_test(data, size2train=size2trainSet, normalFactor=1.0)
    trainSet_i, testSet_i = DNN_data.split_1Data2train_test(data, size2train=size2trainSet, normalFactor=1.0)
    trainSet_r, testSet_r = DNN_data.split_1Data2train_test(data, size2train=size2trainSet, normalFactor=1.0)
    nbatch2train = np.ones(batch_size2train, 1)

    # 从测试集中选出一定的数据作为测试批次。对于时间数据来说，验证模型的合理性，要用连续的时间数据验证
    test_t_bach = DNN_data.sample_testDays_serially(testSet_date, batch_size2test)
    i_obs_test = DNN_data.sample_testData_serially(testSet_i, batch_size2test, normalFactor=1.0)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            # 从训练集中选出数据作为训练批次
            t_batch = DNN_data.seriallySample_existData(trainSet_date, baseIndex=0, batchsize=batch_size2train,
                                                        normalFactor=1.0)
            s_batch = DNN_data.seriallySample_existData(trainSet_s, baseIndex=0, batchsize=batch_size2train,
                                                        normalFactor=1.0)
            i_batch = DNN_data.seriallySample_existData(trainSet_i, baseIndex=0, batchsize=batch_size2train,
                                                        normalFactor=1.0)
            r_batch = DNN_data.seriallySample_existData(trainSet_r, baseIndex=0, batchsize=batch_size2train,
                                                        normalFactor=1.0)
            n_batch = nbatch2train.reshape(batch_size2train, 1)
            ...
            _, s_nn2train, i_nn2train, r_nn2train, beta_nn, gamma_nn, loss_s, loss_i, loss_r, loss_n, loss = sess.run(
                [train_loss, S_NN2train, I_NN2train, R_NN2train, Betas2train, Gammas2train, loss2S, loss2I, loss2R,
                 loss2N, loss_sum], feed_dict={
                    T_batch2train: np.reshape(t_batch, [-1, 1]), S_init2train: s_batch[0], I_init2train: i_batch[0],
                    R_init2train: r_batch[0], S_true2train: s_batch, I_true2train: i_batch, R_true2train: r_batch,
                    N_true2train: n_batch})
            ...
            if i_epoch % 1000 == 0:
                ...
                s_nn2test, i_nn2test, r_nn2test, beta_test, gamma_test = sess.run(
                    [S_NN2test, I_NN2test, R_NN2test, Betas2test, Gammas2test], feed_dict={
                    T_batch2train: np.reshape(t_batch, [-1, 1]), S_init2train: s_batch[0], I_init2train: i_batch[0],
                    R_init2train: r_batch[0], S_true2train: s_batch, I_true2train: i_batch, R_true2train: r_batch,
                    N_true2train: n_batch})
        ...
        saveData.save_SIR_trainLoss2mat_Covid(loss_s_all, loss_i_all, loss_r_all, loss_n_all, actName=actFunc_name,
