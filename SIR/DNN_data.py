...
# 将单个数据集拆分为训练集合测试集
def split_1Data2train_test(data, size2train=50, normalFactor=10000):
    data2train = data[0:size2train]/float(normalFactor)
    data2test = data[size2train:-1]/float(normalFactor)
    return data2train, data2test


# 将两个数据集分别拆分为训练集合测试集
def split_2Data2train_test(date_data, data, size2train=50, normalFactor=10000):
...
# 从数据集中连续采样部分数据作为训练数据
def seriallySample_existData(data_in, baseIndex=0, batchsize=5, normalFactor=1000):
    len2data_in = len(data_in)
    assert(baseIndex+batchsize <= len2data_in)
    data_temp = data_in[baseIndex:, baseIndex+batchsize]
    if normalFactor != 1:
        data_temp .append(float(data_temp)/float(normalFactor))
